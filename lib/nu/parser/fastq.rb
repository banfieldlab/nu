    ...
    class Fastq
      ...
      def initialize(filename = nil)
        ...
        if @filename
          ...
          $stderr.puts("Nu::Parser::Fastq.new(): need an existing fastq file name")
          exit(1)
        end
      end
      ...
      def each
        while @file.eof != true # keep reading until EOF
          ...
          if header =~ /^@(.*)/
            ...
            yield Nu::Sequence::Fastq.new(:header => header, :sequence => sequence, :qualhdr => qualhdr, :quality => quality)
          else
