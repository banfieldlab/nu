# this is a hack of Nu::Parser::Genbank to deal specifically with
# prodigal's limited GFF support
require 'nu/genbank/feature'
require 'nu/genbank/location'
require 'nu/genbank/qualifier'
    ...
    class Prodigal
      ...
      def initialize(filename = nil,debug=false)
        ...
        if filename
          if File.exists?(filename) and File.readable?(filename)
            ...
            error("Nu::Parser::Prodigal.new(): problems with filename")
            raise "File doesn't exist or is not readable!"
          end
        else
          error("Nu::Parser::Prodigal.new(): need a filename")
      ...
      def parse(buffer)
        ...
        if buffer[0] =~ /^DEFINITION\s+(.+)$/
          pseq = Nu::Parser::Prodigal::Sequence.new(:definition => $1)
      ...
      # yielded from Nu::Parser::Prodigal
      class Sequence
        attr_accessor :name, :length, :definition, :features
        ...
        def parse_features(buffer)
          ...
          all_features.each do |feature_str|
            @features << Nu::Genbank::Feature.parse(feature_str)
          end
      ...
      end # end of Nu::Parser::Prodigal::Sequence class
    end # end of Nu::Parser::Prodigal class
