require 'spec_helper'
describe Nu::Sequence do
  before do
    @s = Nu::Sequence.new(:type => 'dna', :sequence => 'atgcatgcatgcatgcaaaa')
  end

  it 'knows about attributes' do
    expect(@s).to be_a(Nu::Sequence)
  ...
  it 'can compute a levenshtein distance' do
    @s.sequence = 'kitten'
    other = Nu::Sequence.new(:value => 'knitting')
  ...
  it 'can determine percent identity' do
    @s.sequence = 'acgt'
    other = Nu::Sequence.new(:value => 'acgg')
  ...
  it 'can find blocks of N characters' do
    sequence_with_nblocks = Nu::Sequence.new(:type     => 'dna',
